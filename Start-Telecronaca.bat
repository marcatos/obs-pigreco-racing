...
echo   3) Session director           (dry-run until you edit config.local.json)
echo.
echo OBS checklist:
echo   - Import/reimport S.Marcato 42 (live) or Replay / Rec 2K after generate_pack
echo   - Live: Monitor Centro + eye ON Overlay Broadcast Chrome
echo   - Dock: http://127.0.0.1:8766/?profile=marcato
echo   - Enable telemetryEnabled (+ trackMapEnabled)
echo   - WebSocket for VirtualDeck: docs\OBS_VIRTUALDECK.md
echo.
echo Docs: docs\TELEMETRY_BROADCAST.md  docs\SESSION_DIRECTOR.md  docs\TRACK_MAP.md
echo.
...
start "PiGreco Session Director" cmd /c "Start-FlagDirector.bat"
echo.
